...
\begin{itemize}
    \item \textbf{Versionado de la aplicaci\'on.} GammaLab 2.0 adopta versionado sem\'antico \texttt{MAJOR.\allowbreak MINOR.\allowbreak PATCH}: \texttt{MAJOR} rompe \texttt{.glab} o el contrato \texttt{IPlugin}; \texttt{MINOR} agrega capacidades retrocompatibles; \texttt{PATCH} corrige sin alterar contratos p\'ublicos. Hoy no existe una constante \texttt{\_\_version\_\_} ni \textit{tags} de Git; se fija como regla que \texttt{main.py} la exponga y quede embebida en cada \texttt{.glab} generado.
    ...
    \item \textbf{Metadatos \texttt{properties.yml}.} Los campos requeridos (\texttt{id, name, category, subcategory, version, icon, logic\_class, ui\_class}) deben existir siempre -- ya exigido por \texttt{REQUIRED\_\allowbreak KEYS} en \texttt{loader.py} -- y a\~nadir campos opcionales es compatible. Regla nueva: \texttt{version} debe declarar el \texttt{MAJOR} de \texttt{IPlugin} contra el que se escribi\'o el plugin; \texttt{discover()} debe rechazarlo si no coincide con el \texttt{MAJOR} del n\'ucleo -- hoy ese campo existe pero no se valida.

    \item \textbf{Formato de proyecto \texttt{.glab} (nuevo).} \texttt{ProjectService} y \texttt{.glab} a\'un no existen en el c\'odigo (cero referencias en el repositorio). Regla de dise\~no: cada \texttt{.glab} debe embeber un \texttt{schema\_version} propio; un \texttt{MINOR} se mantiene legible por versiones futuras, un \texttt{MAJOR} rechaza la apertura con mensaje expl\'icito en vez de migrar impl\'icitamente.
...
La Tabla~\ref{tab:metricas} formaliza la ``Medida de Respuesta'' de cada escenario de calidad (secci\'on~5.1) en una m\'etrica verificable, con su m\'etodo de medici\'on y el criterio de aceptaci\'on correspondiente. Donde ya existe evidencia real (suite \texttt{pytest} o benchmarks de \texttt{test/performance\_test/}), se cita el estado medido a la fecha de este documento en vez de asumir que el criterio se cumple.
...
\texttt{performance\_\allowbreak test}; medido hoy: 78.4\,ms (1.8\,M muestras) &
...
\texttt{test\_\allowbreak export.py} (8/8 pasan hoy) &
...
\texttt{pytest} sobre \texttt{core}/\texttt{plugins} &
$\geq$80\%; hoy 60/60 en \texttt{services\_\allowbreak test}, 0/24 en \texttt{plugins\_\allowbreak test} (import roto) \\
\hline
...
Robustez -- cancelaci\'on (nuevo)\footnotemark &
Liberaci\'on del hilo tras cancelar &
Prueba forzada sobre \texttt{TaskOrchestrator} &
Liberaci\'on garantizada (R46, R70, R74) \\
\hline

\end{longtable}
\end{center}
\footnotetext{El \texttt{TaskOrchestrator} (UC-02, tabla~\ref{tab:uc02}) es un componente nuevo (secci\'on~4.5); esta fila es el criterio de aceptaci\'on para su implementaci\'on, no una medici\'on ya realizada.}
...
\begin{enumerate}
    ...
    \item \textbf{Auditor\'ias documentales.} Los informes \texttt{reporte\_\allowbreak tests.\allowbreak tex} e \texttt{informe\_\allowbreak rendimiento.\allowbreak tex} (\texttt{docs/\allowbreak tests documents/}) registran el estado real de la suite y los hallazgos de rendimiento a la fecha de este documento.
    ...
    \item \textbf{Registro de cancelaci\'on forzada.} Una vez implementado el \texttt{TaskOrchestrator}, su prueba de cancelaci\'on bajo carga queda como evidencia de Robustez (UC-02).
\end{enumerate}

\subsection{Conclusi\'on}

Los escenarios de calidad, las reglas de compatibilidad y la tabla de m\'etricas convierten los atributos de calidad de la secci\'on~3.2 en criterios operativos y verificables, no en aspiraciones gen\'ericas. A la fecha de este documento, tres brechas concretas quedan abiertas y deben cerrarse antes de que varios criterios de la Tabla~\ref{tab:metricas} puedan darse por cumplidos: el import roto que impide ejecutar \texttt{plugins\_\allowbreak test} (Mantenibilidad), la ausencia de integraci\'on continua que hubiera detectado esa ruptura antes, y los componentes a\'un no implementados (\texttt{TaskOrchestrator}, \texttt{ProjectService}/\texttt{.glab}) de los que dependen Robustez y Trazabilidad. El plan de evidencia de la secci\'on~6.4 y las reglas de versionado de la secci\'on~6.2 fijan c\'omo y con qu\'e artefactos se demuestra, de forma reproducible, que estas brechas se cerraron.
